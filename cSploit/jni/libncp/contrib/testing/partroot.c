/*
    partroot.c - NWDSGetPartitionRoot() API demo
...
int main(int argc, char *argv[]) {
	...
	char partitionRoot[MAX_DN_CHARS+1];
	int opt;
	u_int32_t ctxflag = 0;
	...
	dserr = NWDSGetPartitionRoot(ctx, objectname, partitionRoot);
	if (dserr) {
		fprintf(stderr, "NWDSGetPartitionRoot failed with %s\n", 
			strnwerror(dserr));
	} else {
		printf("%s lives in partition %s\n", objectname, partitionRoot);
	}
	NWCCCloseConn(conn);
