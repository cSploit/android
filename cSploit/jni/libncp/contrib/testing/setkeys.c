/*
    setkeys.c - NWDSGenerateObjectKeyPair() API demo
...
help(void)
{
	...
	printf(_("\n"
	       ...
	       "-P password     New password (default TEST)\n"
	       "\n"));
}
...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "h?o:c:v:S:C:P:")) != EOF)
	{
		switch (opt)
		{
		case 'C':
	...
	dserr = NWDSGenerateObjectKeyPair(ctx, objectname, pwd, NDS_PASSWORD);
	if (dserr) {
		fprintf(stderr, "Cannot generate object key pair: %s\n",
			strnwerror(dserr));
	}
	NWCCCloseConn(conn);
