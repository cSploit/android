/*
    renobj.c - NWDSModifyRDN() API demo
...
help(void)
{
	...
	printf(_("\n"
	       ...
	       "-q deleteOld    Delete old RDN (default 0)\n"
...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "h?o:c:v:S:C:n:q:")) != EOF)
	{
		switch (opt)
		{
		case 'C':
	...
	dserr = NWDSModifyRDN(ctx, objectname, newrdn, flag);
	if (dserr) {
		fprintf(stderr, "ModifyRDN failed with %s\n", 
			strnwerror(dserr));
	} else {
		printf("ModifyRDN suceeded\n");
	}
	NWCCCloseConn(conn);
