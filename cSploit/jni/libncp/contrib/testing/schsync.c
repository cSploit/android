/*
    schsync.c - NWDSSyncSchema() API demo
	...
	1.00  2000, May 8		Petr Vandrovec <vandrove@vc.cvut.cz>
		Initial revision.
...
help(void)
{
	...
	printf(_("\n"
	       ...
	       "-o object_name  Name of server (default [Root])\n"
...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "h?o:c:v:S:C:t:")) != EOF)
	{
		...
		case 'v':
			...
			break;
		case 't':
			syncdelay = strtoul(optarg, NULL, 0);
			break;
		case 'h':
		case '?':
			help();
	...
	dserr = NWDSSyncSchema(ctx, objectname, syncdelay);
	if (dserr) {
		fprintf(stderr, "SyncSchema failed with %s\n", 
			strnwerror(dserr));
	} else {
		printf("SyncSchema suceeded\n");
	}
	NWCCCloseConn(conn);
