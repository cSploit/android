/*
    effright.c - NWDSGetEffectiveRights API demo
...
help(void)
{
	...
	printf(_("\n"
	       ...
	       "-s subject_name Name of subject (default [Root])\n"
	       "-o object_name  Name of object (default [Root])\n"
	       "-a attr_name    Name of attribute (default Object Class)\n"
...
int main(int argc, char *argv[]) {
	...
	const char* subjectname = "[Root]";
	const char* attrname = "Object Class";
	int opt;
	u_int32_t ctxflag = 0;
	...
	dserr = NWDSGetEffectiveRights(ctx, subjectname, objectname,
			attrname, &privilege);
	if (dserr) {
		fprintf(stderr, "NWDSGetEffectiveRights failed with %s\n", strnwerror(dserr));
	} else {
		printf("Rights of %s to %s(%s) are %08X\n",
			subjectname, objectname, attrname, privilege);
	}
	NWCCCloseConn(conn);
