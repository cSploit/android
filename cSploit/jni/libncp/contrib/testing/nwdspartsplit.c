/*
    nwdspartsplit.c - NWDSSplitPartition() API demo
	...
	1.00  2000, April 28		Petr Vandrovec <vandrove@vc.cvut.cz>
		Initial revision.
...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "h?o:c:v:S:C:q:")) != EOF)
	{
		switch (opt)
		{
		case 'C':
	...
	dserr = NWDSSplitPartition(ctx, objectname, req);
	if (dserr) {
		fprintf(stderr, "SplitPartition failed with %s\n", 
				strnwerror(dserr));
	}
	NWCCCloseConn(conn);
