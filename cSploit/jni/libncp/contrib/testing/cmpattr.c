/*
    cmpattr.c - NWDSCompare() API demo
...
help(void)
{
	...
	printf(_("\n"
	       ...
	       "-a attr_name    Attribute name (default Object Class)\n"
	       "-V value        Attribute Value (default Partition)\n"
	       "\n"));
}
...
int main(int argc, char *argv[]) {
	...
	dserr = NWDSPutAttrName(ctx, buf, attrname);
	if (dserr) {
		fprintf(stderr, "Cannot put attrname: %s\n",
			strnwerror(dserr));
	}
	dserr = NWDSPutAttrVal(ctx, buf, SYN_CI_STRING, val);
	if (dserr) {
		fprintf(stderr, "Cannot put value: %s\n",
			strnwerror(dserr));
	}
	dserr = NWDSCompare(ctx, objectname, buf, &bol);
	if (dserr) {
		fprintf(stderr, "Compare failed with %s\n", 
			strnwerror(dserr));
	} else {
