/*
    nwdsresetstat.c - NWDSResetNDSStatistics() API demo
...
int main(int argc, char *argv[]) {
	...
	dserr = NWDSResetNDSStatistics(ctx, objectname);
	if (dserr) {
		fprintf(stderr, "NWDSResetNDSStatistics failed with %s\n", 
			strnwerror(dserr));
	} else {
		printf("NWDSResetNDSStatistics suceeded.\n");
	}
#ifdef N_PLAT_LINUX	
	NWCCCloseConn(conn);
#endif
