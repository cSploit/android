/*
    mvobj.c - NWDSMoveObject() API demo
...
help(void)
{
	...
	printf(_("\n"
	       ...
	       "-o object_name  Name of object (default XAVIER)\n"
	       "-p parent_dn    New parent of object (default [Root])\n"
	       "-n new_rdn      New RDN of object (default RENAMED)\n"
...
int main(int argc, char *argv[]) {
	...
	NWCONN_HANDLE conn;
	const char* objectname = "XAVIER";
	const char* parentDN = "[Root]";
	const char* context = "OrgUnit.Org.Country";
	const char* server = "CDROM";
	const char* newrdn = "RENAMED";
	int opt;
	u_int32_t ctxflag = 0;
	...
	while ((opt = getopt(argc, argv, "h?o:c:v:S:C:n:p:")) != EOF)
	{
		switch (opt)
		{
		case 'C':
	...
	dserr = NWDSMoveObject(ctx, objectname, parentDN, newrdn);
	if (dserr) {
		fprintf(stderr, "MoveObject failed with %s\n", 
			strnwerror(dserr));
	} else {
		printf("MoveObject suceeded\n");
	}
	NWCCCloseConn(conn);
