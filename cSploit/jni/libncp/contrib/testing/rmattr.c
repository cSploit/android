/*
    rmattr.c - NWDSRemoveAttrDef() API demo
...
help(void)
{
	...
	printf(_("\n"
	       ...
	       "-o attr_name   Name of class (default Top)\n"
...
int main(int argc, char *argv[]) {
	...
	dserr = NWDSRemoveAttrDef(ctx, objectname);
	if (dserr) {
		fprintf(stderr, "RemoveAttrDef failed with %s\n", 
			strnwerror(dserr));
	} else {
		printf("RemoveAttrDef suceeded\n");
	}
	NWCCCloseConn(conn);
