/*
    readcc.c - NWDSListContainableClasses() API demo
...
int main(int argc, char *argv[]) {
	...
	do {
		dserr = NWDSListContainableClasses(ctx, objectname, &ih, buf);
		if (dserr) {
			fprintf(stderr, "NWDSListContainableClasses failed with %s\n", 
				strnwerror(dserr));
		} else {
			unsigned char* pp;
			NWObjectCount cnt;
			...
			pp = buf->curPos;		
			dserr = NWDSGetClassItemCount(ctx, buf, &cnt);
			if (dserr) {
				fprintf(stderr, "GetClassItemCount failed with %s\n",
					strnwerror(dserr));
			} else {
				printf("Classes: %u\n", cnt);
				pp = buf->curPos;
				while (cnt--) {
					...
					if (dserr) {
						fprintf(stderr, "GetClassItem failed with %s\n",
							strnwerror(dserr));
						break;
					}
					printf("  Class: '%s'\n", name);
