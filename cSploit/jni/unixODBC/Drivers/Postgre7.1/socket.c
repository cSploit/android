...
struct sockaddr_un sadr;
	...
	sprintf( sadr.sun_path, "%s.%d", path, port );
...
SOCK_connect_to(SocketClass *self, unsigned short port, char *hostname, char *uds)
{
	...
	if ( strcmp( hostname, "localhost" ) == 0 )
		return SOCK_connect_to_unix( self, port, uds );
	else
#endif
...
#ifdef MSG_NOSIGNAL
    written = send(self->socket, (char *)self->buffer_out, self->buffer_filled_out, MSG_NOSIGNAL);
#else
