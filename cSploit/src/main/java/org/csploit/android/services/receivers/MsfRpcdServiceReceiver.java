...
public class MsfRpcdServiceReceiver extends ManagedReceiver {
    ...
    public void onReceive(final Context context, Intent intent) {
        ...
        if (context instanceof Activity) {
            ((Activity) context).runOnUiThread(() -> showToastForStatus(context, status));
        } else {
    ...
    private void showToastForStatus(Context context, MsfRpcdService.Status status) {
        Snackbar.make(((AppCompatActivity) context)
                .findViewById(android.R.id.content), status.getText(), status.isError() ?
