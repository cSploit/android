...
public class LoginCracker extends Plugin
{
  ...
  public boolean onOptionsItemSelected(MenuItem item){
    ...
    switch(itemId){
      case R.id.user_wordlist:
        ...
        startActivityForResult(Intent.createChooser(mWordlistPicker, getString(R.string.select_wordlist)), SELECT_USER_WORDLIST);

        return true;

      case R.id.pass_wordlist:

        startActivityForResult(Intent.createChooser(mWordlistPicker, getString(R.string.select_wordlist)), SELECT_PASS_WORDLIST);
  ...
  public class ProtocolAdapter extends BaseAdapter implements SpinnerAdapter{
    ...
    public View getView(int position, View convertView, ViewGroup parent){
      ...
      TextView textView = (TextView) (spinView != null ? spinView.findViewById(android.R.id.text1) : null);

      String sProtocol = mProtocols.get(position);

      if(textView != null) textView.setText(sProtocol);
  ...
  private class AttemptReceiver extends Hydra.AttemptReceiver{
    ...
    public void onNewAttempt(String login, String password, final int progress, final int total){
      ...
      if(mLastAttempt == 0)
        ...
        status += "\t[ " + formatTimeLeft(mLastLeft) + " " + getString(R.string.left) + " ]";
      }
