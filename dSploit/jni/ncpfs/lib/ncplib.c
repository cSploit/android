...
static int get_argument(int arg_no, const char **target, int *argc, char **argv) {
	int count = 1;
		 ...
		 int login_type, long *err, int required)
{
	...
	*err = EINVAL;
	...
	while (i < *argc) {
		...
		switch (argv[i][1]) {
		case 'S':
			if (get_argument(i, &server,argc,argv) != 0) {
				return NULL;
			}
			continue;
		case 'U':
			if (get_argument(i, &user,argc,argv) != 0) {
				return NULL;
			}
			continue;
		case 'P':
			if (get_argument(i, &password,argc,argv) != 0) {
				return NULL;
			}
			if (password) {
			...
			continue;
		case 'n':
			if (get_argument(i, NULL,argc,argv) != 0) {
				return NULL;
			}
...
#ifdef CONFIG_NATIVE_IP
		case 'A':
			if (get_argument(i, &address,argc,argv) != 0) {
				return NULL;
			}
			continue;
#endif	/* CONFIG_NATIVE_IP */
