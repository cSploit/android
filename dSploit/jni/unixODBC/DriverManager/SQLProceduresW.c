 ...
 **********************************************************************
 *
 * $Id: SQLProceduresW.c,v 1.9 2009/02/18 17:59:08 lurcher Exp $
 *
    ...
    if ( !__validate_stmt( statement ))
    {
...
#ifdef WITH_HANDLE_REDIRECT
		{
			...
			if ( parent_statement ) {
				...
				if ( CHECK_SQLPROCEDURESW( parent_statement -> connection ))
				{
        			dm_log_write( __FILE__, 
                		__LINE__, 
                   		 	LOG_INFO, 
                	...
                	return  SQLPROCEDURESW( parent_statement -> connection,
							statement_handle,
    						sz_catalog_name,
    						cb_catalog_name,
    						sz_schema_name,
...
\n\t\t\tProc Name = %s",
    ...
    if ( statement -> connection -> unicode_driver ||
		    CHECK_SQLPROCEDURESW( statement -> connection ))
    {
